...
def graph(green,total):
    if not total:
        return ('<div style="width: 100px; height: 10px; border:1px solid black"></div>')
    percentage_passed=int(green*100.0/total)
    return ('<div style="width: 100px; height: 10px; border: 1px solid black; background: red"><div style="width: '+str(percentage_passed)+'px; height: 10px; background: green"></div></div>')
#---------------------------admins
