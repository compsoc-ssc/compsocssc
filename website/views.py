...
def members(request):
    '''Members page'''
    context = {}
    context['current'] = models.CompMember.objects.filter(alumni=False).order_by('-index')
    context['alumni'] = models.CompMember.objects.filter(alumni=True).order_by('-batch_of', '-index')
